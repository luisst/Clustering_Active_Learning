# filepath: c:\Users\luis2\Dropbox\Source_2025\Python_scripts\hdbscan_demo.py
...
def create_color_mapping(labels, color_palette='tab10', outlier_color='black'):
    """
    Create a consistent color mapping for cluster labels.
    
    Args:
        labels: array of cluster labels
        color_palette: matplotlib colormap name
        outlier_color: color for outlier points (label -1)
    
    Returns:
        dict: mapping from label to color
    """
    unique_labels = np.unique(labels)
    unique_labels = unique_labels[unique_labels != -1]  # Remove outliers for color assignment
    
    # Get colors from colormap
    if len(unique_labels) > 0:
        colors = plt.cm.get_cmap(color_palette)(np.linspace(0, 1, len(unique_labels)))
...
def plot_prediction_clusters(X, labels, wav_names=None, probabilities=None, parameters=None, 
                           ax=None, txt_title=None, remove_outliers=True, color_map=None):
    """
    ...
    if color_map is None:
        color_map = create_color_mapping(labels)
    ...
    for label in unique_labels:
        ...
        color = color_map.get(label, 'gray')  # Use gray as fallback
...
def plot_clustering_hdb(X, labels, probabilities=None, parameters=None, 
                    ...
                    remove_outliers = True, color_map=None):
    ...
    for k in unique_labels:
        if k == -1:
            ...
            col = color_map.get(k, [0, 0, 0, 1])
            if remove_outliers:
                continue
        else:
            col = color_map.get(k, [0.5, 0.5, 0.5, 1])  # Fallback to gray
        ...
        # Add to legend
        if k != -1 or not remove_outliers:
            legend_without_symbol.append(mlines.Line2D([], [], color=col,
                                                        marker='o' if k != -1 else 'x', 
                                                        linestyle='None',
                                                        label=f": {list(labels).count(k)}"))
...
if __name__ == "__main__":
    ...
    pred_color_map = create_color_mapping(hdb_labels, color_palette='tab10')
    gt_color_map = create_color_mapping(y_labels, color_palette='Set1')
    ...
    df = pd.DataFrame({
        ...
        "gt_label": y_labels,
        "label": hdb.labels_,  # Fixed: was "pred_label"
    ...
    # Plot knn_entropy_k5 as y-axis and knn_distance_k5 as x_axis with consistent colors
    plt.figure(figsize=(12, 8))
    # Create a color array based on the color mapping
    colors_for_scatter = [pred_color_map.get(label, 'gray') for label in hdb.labels_[df.index]]
    plt.scatter(df["knn_distance_k5"], df["knn_entropy_k5"], c=colors_for_scatter, alpha=0.5)
...
def custom_label_propagation(X, hdbscan_model, human_labels, alpha=0.9, max_iter=100, tol=1e-4):
    """
    Semi-supervised label propagation using HDBSCAN graph with mutual reachability distances.
    
    Parameters
    ----------
    X : np.ndarray, shape (n_samples, n_features)
        Original feature embeddings (e.g., 256-dim speaker embeddings).
    
    hdbscan_model : fitted hdbscan.HDBSCAN instance
        Pre-trained HDBSCAN model with attributes labels_ and probabilities_.
    
    human_labels : dict
        Mapping {sample_index: speaker_id} with human-provided ground-truth labels.
    
    alpha : float, default=0.9
        Propagation strength. Closer to 1 = smoother propagation, 
        closer to 0 = stick more to initial labels.
    ...
    y_pred : np.ndarray, shape (n_samples,)
        Final propagated speaker labels.
    label_distributions : np.ndarray, shape (n_samples, n_classes)
        Probability distribution over speakers for each sample.
    """
    ...
    # Get k value from HDBSCAN model (min_samples parameter)
    k = getattr(hdbscan_model, 'min_samples', 5)
    
    # --- Step 1: Calculate core distances (distance to k-th nearest neighbor) ---
    
    # Fit nearest neighbors to find k-nearest neighbors for each point
    nbrs = NearestNeighbors(n_neighbors=k, metric='euclidean').fit(X)
    distances, indices = nbrs.kneighbors(X)
    
    # Core distance is the distance to the k-th nearest neighbor (last distance in each row)
    core_distances_calculated = distances[:, -1]  # k-th nearest neighbor distance
    
    # --- Verification step: Compare with HDBSCAN's internal core distances ---
    hdbscan_core_distances = hdbscan_model._prediction_data.core_distances
    print(f"Core distances comparison:")
    print(f"  Our calculation mean: {np.mean(core_distances_calculated):.6f}")
    print(f"  HDBSCAN internal mean: {np.mean(hdbscan_core_distances):.6f}")
    print(f"  Max absolute difference: {np.max(np.abs(core_distances_calculated - hdbscan_core_distances)):.6f}")
    print(f"  Are they close? {np.allclose(core_distances_calculated, hdbscan_core_distances, rtol=1e-5)}")

    
    # --- Step 2: Build graph adjacency using mutual reachability distances ---
    # For efficiency, we'll build the graph using k-nearest neighbors
    # and calculate mutual reachability distances only for these connections
    
    # Extend to more neighbors for better graph connectivity
    k_graph = k

    nbrs_graph = NearestNeighbors(n_neighbors=k_graph, metric='euclidean').fit(X)
    graph_distances, graph_indices = nbrs_graph.kneighbors(X)
    
    row_idx, col_idx, mutual_reach_distances = [], [], []
    
    for i in range(n_samples):
        for j_idx, original_dist in enumerate(graph_indices[i]):
            j = original_dist
            if i != j:  # Skip self-connections
                original_distance = graph_distances[i][j_idx]
                
                # Calculate mutual reachability distance
                # d_mreach(a,b) = max(core_k(a), core_k(b), d(a,b))
                mutual_reach_dist = max(
                    core_distances_calculated[i],      # core_k(a)
                    core_distances_calculated[j],      # core_k(b)
                    original_distance                  # d(a,b)
                )
                
                # Convert distance to similarity (higher similarity for smaller distances)
                similarity = np.exp(-mutual_reach_dist)
                
                row_idx.append(i)
                col_idx.append(j)
                mutual_reach_distances.append(similarity)
    
    # Create sparse adjacency matrix
    W = sp.csr_matrix((mutual_reach_distances, (row_idx, col_idx)), shape=(n_samples, n_samples))
    
    # Make the matrix symmetric (optional, for undirected graph)
    W = (W + W.T) / 2

    # Add small self-connections before normalization
    W.setdiag(0.1)  # Small self-weight
    
    # Row-normalize for propagation
    W = normalize(W, norm="l1", axis=1)
    
    print(f"Graph statistics:")
    print(f"  Number of edges: {len(mutual_reach_distances)}")
    print(f"  Average degree: {len(mutual_reach_distances) / n_samples:.2f}")
    print(f"  Mutual reachability distances - min: {np.min([1/s for s in mutual_reach_distances]):.6f}, max: {np.max([1/s for s in mutual_reach_distances]):.6f}")
    
    # --- Step 3: Initialize label distributions ---
    ...
    for idx, spk in human_labels.items():
        Y[idx, id_to_idx[spk]] = 1.0  # one-hot for labeled nodes
    
    print(f"Label propagation setup:")
    print(f"  Number of labeled samples: {len(human_labels)}")
    ...
    for it in range(max_iter):
        F_new = alpha * W.dot(F) + (1 - alpha) * Y  # smooth + clamp
...
# --- Helper function to test the mutual reachability calculation ---
def verify_mutual_reachability_calculation(X, hdbscan_model, sample_indices=None):
    """
    Verify our mutual reachability distance calculation against HDBSCAN internals.
    ...
    hdbscan_model : fitted HDBSCAN model
        Model with prediction_data=True
    sample_indices : list, optional
        Specific sample indices to check (default: first 10)
    """
    ...
    nbrs = NearestNeighbors(n_neighbors=k, metric='euclidean').fit(X)
    distances, indices = nbrs.kneighbors(X)
    our_core_distances = distances[:, -1]
    
    # HDBSCAN's calculation
    try:
        hdbscan_core_distances = hdbscan_model._prediction_data.core_distances
        
        print("Core distance verification:")
        print("Sample | Our Core | HDBSCAN Core | Difference")
        print("-" * 50)
        for i in sample_indices:
            diff = abs(our_core_distances[i] - hdbscan_core_distances[i])
            print(f"{i:6d} | {our_core_distances[i]:8.6f} | {hdbscan_core_distances[i]:8.6f} | {diff:8.6f}")
        
        return np.allclose(our_core_distances, hdbscan_core_distances, rtol=1e-5)
        
    except AttributeError:
        print("HDBSCAN internal core distances not available")
        return False
