...
from Stage1_utils import vad_format, select_method
from pipeline_utilities import valid_path, log_print, find_audio_duration, matching_basename_pathlib_gt_pred
...
def vad_calculation(pred_method,
    ...
    for current_gt_pth, current_pred_pth in current_matches:
        ...
        log_print(f'\n\n\t\t{pred_method}\t{current_gt_pth.stem} - {total_speech:.2f}s / {current_audio_duration:.2f}s', lp=current_log)
        log_print(f'\tAcc: {accuracy_val:.2f}%\tMissed_speech: {missed_speech_rate}% ({missed_speech_val:.1f}s)\tFalse_speech: {false_speech_rate:.1f}% ({false_speech_val:.1f}s)\t|\tPrec: {precision_rate_val:.1f}%\tRecall: {recall_rate_val:.1f}%', lp=current_log)

    # Calculate the average of all the metrics
    all_vad_metric = pd.DataFrame(all_vad_metric)
    all_vad_metric = all_vad_metric.mean()

    log_print(f"\n\n{pred_method}\tAverage Metrics Values:", lp=current_log)
...
verbose = False
mylog = str(metric_output_folder.joinpath(f'VAD_metrics_{run_name}.txt'))
